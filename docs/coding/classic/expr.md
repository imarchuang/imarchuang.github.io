...
> 单个字符串做输入，或者一堆字符串 array 做输入，你的思路不外乎以下几种：
>
> 1.  动规思路，刷到今天，这种题型应该很容易辨别出来了，就是**蛙跳**嘛，基本上都是子序列相关的问题，经典的例子像[最长回文子序](https://leetcode.com/problems/longest-palindromic-subsequence/)之类的。当然了，类似思路还有更原始的题型，比如说[找字符串中符合 word 的子序列](#https://www.lintcode.com/problem/1024/)，这子序类的思路在[这篇](./coding/classic/subsequence)详细讲解过，请温习。
> 1.  第二类思路呢，题型基本上就是子串类的，然后核心思路就是二分字符串，一头一尾，头部是`s.substring(0, i+1)`和尾部`s.substring(i+1)`，处理头部并递归尾部，这个思路特别好使，你可以秒杀相对复杂的题如单词模式 II，单词拆分 II 等。
...
>
> - 给你一个类似`3[2[ad]3[pf]]xyz`这样一个字符串，让你把这个展成`"adadpfpfpfadadpfpfpfadadpfpfpfxyz"`，这个怎么**掏心**法呢？你脑子是否有个栈？这个栈呢存这么样的数据`3|2|a|d|`，现在你遇到右括号`]`字符了，说明当前层结束，你要进行 aggregate 了。aggregate 到啥时候结束呢？这里就是根据栈内元素是否是个数字；
> - 给你一个化学式`"K4(ON(SO3)2)2"`这样一个字符串，让你把这个展成字典序的`"K4N2O14S4""`，这个怎么**掏心**法呢？循规蹈矩，你脑子应该有个栈。这个栈呢存这么样的数据`{K:4}|{N:1; O:1}|{O:3； S:1}|`，现在你遇到右括号`)`字符了，说明当前层结束，你要进行 aggregate 了。咋个 aggregate 法呢？这时候你要把右括号`)`后跟的那个数字找出来，然后把栈顶的 TreeMap 拿出来，对 TreeMap 里的每个元素都乘以这个数字；这里不需要特殊处理**层**的结束点，因为你在压栈过程中保证每层(即每遇到左括号`(`时候)就新建一个 TreeMap，这样说明栈顶的 TreeMap 就是当前层的所有元素；
...
> - 给你一个类似`"Bryce,Beverly","kid\"s"`这样一个 csv 格式的字符串，让你把这个 parse 成`[Bryce,Beverly,kid"s]`，这个怎么**掏心**法呢？这个呢其实不算**掏心**，因为没有设计**层**的概念，你可以基本上从左往右扫描一遍就能解决了。
