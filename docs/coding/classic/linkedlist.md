...
> 链表和数组的一些技巧上有非常多的共同之处，链表类的题目基本上都是线性的，因为这个**集合**内的元素自带一个next指针，就决定了你玩不上很多花样来。其实呢，链表具备两种属性，一种叫**粒子性**，也就是说链表上的节点都是若干个**独立的**元素；另一种属性叫**递归性**，也就是说每个链表都是一个头结点head加上一个**子链表head.next**，下面会展开看这两种属性。
>
