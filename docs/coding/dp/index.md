...
<iframe src="/drops/algo-lab/?note=dp-index&embed=1" title="算法交互演示" width="100%" height="720" style="border:0;width:100%;height:720px;background:#16382b;"></iframe>

> 建议想深入动规系列之前，先把二叉树的这几篇帖子温习一下[二叉树总结](./coding/tree/index)、[二叉树遍历](./coding/tree/traversal)、[二叉树后序](./coding/tree/postorder)，还有温习一下这个[分治的帖子](./coding/dfs/divcon)，因为这都是跟动规一脉相承的。
...
> **更新(2022-10-16)**: 我们说动规题的套路是从二叉树那里的**分治思路**衍生来的，那里的分治思想就是能够将问题切的越小越好直到base case，二叉树里的base case通常是基于leaf node的展开的。二叉树里的分治框架也比较直观，通常是求出左子树的统计信息，再求出右子树的统计信息，然后在**后序位置**连同当前节点将需要的统计信息roll up起来然后返回*递归树*的上一层。
>
> 后来我们把这个**分治思路**延伸到`单词拆分II`，`单词模式II`等题上，那里的难点是把一个字符串抽象成**二分树**，头部是`s.substring(0, i+1)`和尾部`s.substring(i+1)`。
>
> 再后来我们还把这个**分治思路**延伸到**掏心算法**题上，比如`求表达式的值`，`原子的数量`等他，只不过那里的子问题的定义是要自己显性的去用*开始标识*和*结束标识*去框定的。
>
> 上面的几个用**分治思路**解题的过程中，我们并没有涉及到**重复子问题**和**最优子结构**的讨论上，因为那些题的子问题都是*独立的*，所以用**暴力美学**式的穷举是很奏效的。而动态规划的题呢，虽然核心还是**分治思路**，但是因为牵扯到**重复子问题**和**最优子结构**，怎么界定**子问题**符合最优子结构并证明可以从最优子结构roll up得到最终答案就变成了难点，这里说的即是**状态转化方程**，这个转化即是roll up子问题的小范围答案到最终的原题意的答案。
>
>
...
1. **`dp(n)`**: 是说给一数列[1...n]，能得到多少个二叉搜索树，这也是原题要找的答案；
1. **`f(i,n) & 1<=i<=n`** ：这个函数是说，以数字`i`为根，一共有`n`个node分别是`[1,2,...n]`，能得到多少个二叉搜索树；
1. 通过以上两个函数，我们不难发现其实主要是对`f(i,n)`中的`i`和`n`进行合理的遍历，这两变量就是所谓的`状态`，两个**状态**嘛，所以要从两个维度遍历这两个状态的所有可能值；
1. 动规的问题中，其实还有一块要注意，就是所谓的`选择`。这里选择比较明确，就是将`(i,n)`两变量按照每步为1的节奏遍历，以穷举完 **`1<=i<=n`** 并且**`1<=j<=i`**；
1. 上边两个函数的关系就是：`dp(n) = f(1, n) + f(2, n) + ... + f(n, n)`;这就是所谓的`状态转移方程`；
1. **动规问题都是穷举**嘛，但是穷举必须有出口啊。这就是所谓的`base case`，这里的base case是`dp(0)=1`，`dp(1)=1`；
