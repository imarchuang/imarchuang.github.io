...
### 背包型动规题长啥样，或者说可以转化成这样吗？
> 1. 给出 n 个物品及其大小(`可选择的物品w[i]`),问是否能挑选出一些物品装满大小为 m 的背包(`背包的容量W`)；
> 1. 题目中通常有`和`与`差`的概念,数值会被放到状态中;
> 1. 通常是二维的状态数组`dp[i][j]`, 表示`前 i 个`组成`和为 j 状态`数组的大小需要开 (n + 1) * (m + 1); 下面是几个常见的例子：
>       * 能不能用前i个物品将容量j填满？换种说法：能不能从`前 i 个数`里挑`若干个数`组成`和为 j `的子数组？
>       * 从`前 i 个数`里挑`若干个数`组成的子数组里，在不超过`容量 j `的情况，哪种子数组之和值最大？
>       * 从前i个物品选择，将背包填满的选择一共有几种？换种说法：从`前 i 个数`里挑`若干个数`组成`和为 j `的子数组，这种子数组组合一共有几种可能？

> **具体实例点** 针对以上三种情况，我再用实际例子来具象化一下：
> 1. **例**：(a) 给你3个物品，重量分别是1kg,2kg,3kg; (b) 给你一个背包，容量为3kg。
>       * 能不能从`前 3 个数`里挑`若干个数`组成`和为 3(kg) `的子数组？答案是可以啊，填满背包其实有两种可能：1. 装入1kg和2kg的物品；2. 装入3kg的那个物品；
>       * `前 i 个数`里挑`若干个数`组成的子数组里，在不超过`容量 j `的情况，哪种子数组之和值最大？这里我们思考一下遍历i维度的情况：前0个物品，最大可以装入0kg的量；前1个物品，最大可以装入1kg的量；前2个物品，最大可以装入3kg(装入1kg和2kg的物品)的量；前3个物品，最大可以装入3kg(装入1kg和2kg的物品，或者装入3kg的那个物品)的量；**你体会到了吗？** 我们使用整数颗粒度1（因为是物品的个数嘛）来遍历的，在i维上从0到3递增遍历，**记住是前i个物品，所以3表示从前三个里选最优** 与之对应的dp[...][1]的数值就应该是这样的：`dp[...][1] = [0kg], [1kg], [3kg], [3kg]`;
>       * 从`前 3 个数`里挑`若干个数`组成`和为 3(kg) `的子数组，这种子数组组合一共有几种可能？这里答案很明显，就是两种(装入1kg和2kg的物品，或者装入3kg的那个物品)。**思考一下** 前3个物品，组成0kg量的组合有0种；前3个物品，组成1kg量的组合有1种；前3个物品，组成2kg量的组合有1种；前3个物品，组成3kg量的组合有1种；**你体会到了吗？** 我们使用整数颗粒度1kg（因为是物品的重量都是以整数给出）来遍历的，说白了就是穷举在各个kg粒度的可能性（最大是原来背包3kg），与之对应的dp[...][3]的数值就应该是这样的：`dp[...][1] = [0], [1], [1], [2]`; 

这样我们就可以在j维上从0到3递增遍历，与之对应的dp[1]的数值就应该是这样的：`dp[1] = [0kg, 0kg, 2kg, 2kg]`; 

!> **敲黑板1** 背包问题的`状态`就是`背包的容量W`和`可选择的物品w[i]`，与这个些物品对应的，通常会有物品的对应价值v[i]，用这个价值来选择最优解。至于`选择`嘛很直接，就是`装入背包`和`不放人背包`。

!> **敲黑板2** 背包问题通常学要一个二维数组`dp[i][j]`来记录状态。i维是物品选择，表示前i个物品作为选择；j维是背包容量。物品的个数嘛，颗粒度肯定是1的，所以i维的话就能把物品缩小到1和没有（0为base case），至于j维嘛，给出的物品选择的重量通常是整数的，所以背包容量也是可以以整数1的颗粒度来进行穷举的。

![](./pictures/knapsack.jfif)

### 背包型问题的状态转化方程
> 铺垫了这么多，应该能理解状态数组`dp[i][j]`就是表示`前 i 个`组成`和为 j 状态`数组的大小，然后`选择`就两种：放入背包或者不放入背包，那么在二维上从小到大遍历过程中，状态转化大体上都是张这个样子的：
```java
// 0/1背包 
...
-----------------------------------------------------------------------------------------------------
### **打家劫舍类
!> **敲黑板** 打家劫舍类不算背包问题，但背后的思路很相似。打家劫舍问题其实也是子集类问题，所以吧都算是聪明的穷举子集问题，只不过打家劫舍类是`有限制的子集`，所以有点像是在0/1背包问题是加了限制条件的感觉。

1. [198 打家劫舍i](#打家劫舍I)
1. [213 打家劫舍ii](#打家劫舍II)
1. [337 打家劫舍iii](#打家劫舍III)
